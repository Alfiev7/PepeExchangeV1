        ...
        <div className="px-4 sm:px-0">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
              ...
              <div className="text-left sm:text-right w-full sm:w-auto">
                ...
                <p className="text-xl sm:text-2xl font-bold text-white">
                  {formatCurrency(
                    user.balance +
                      Object.entries(user.holdings).reduce(
                        (total, [coinId, amount]) => {
                          const coin = coins.find((c) => c.symbol === coinId);
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {coins.map((coin) => (
              <div
                key={coin._id}
                className="bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-lg shadow-lg overflow-hidden"
              >
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <input
                        ...
                        onChange={(e) =>
                          handleBuyAmountChange(coin._id, e.target.value)
                        }
                      ...
                      {buyAmounts[coin._id] && (
                        <p className="text-sm text-gray-400 mt-1">
                          ≈{" "}
                          {formatCurrency(
                            parseFloat(buyAmounts[coin._id]) * coin.price
                          )}
                        </p>
                      )}
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (amount > 0)
                            handleTransaction(coin.symbol, "buy", amount);
                        }}
                        className={`w-full mt-2 px-4 py-2 rounded-md ${
                          loading
                            ? "bg-gray-500 cursor-not-allowed"
                            : "bg-green-500 hover:bg-green-600"
                        } text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition duration-150 ease-in-out`}
                      >
                        Buy
                      </button>
                    </div>
                    <div>
                      ...
                      {sellAmounts[coin._id] && (
                        <p className="text-sm text-gray-400 mt-1">
                          ≈{" "}
                          {formatCurrency(
                            parseFloat(sellAmounts[coin._id]) * coin.price
                          )}
                        </p>
                      )}
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (amount > 0)
                            handleTransaction(coin.symbol, "sell", amount);
                        }}
                        className={`w-full mt-2 px-4 py-2 rounded-md ${
                          loading
                            ? "bg-gray-500 cursor-not-allowed"
                            : "bg-red-500 hover:bg-red-600"
                        } text-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition duration-150 ease-in-out`}
                      >
                        Sell
                      </button>
                    </div>
